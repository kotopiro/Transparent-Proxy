    ...
    event.respondWith((async()=>{
      try{
        ...
        if(ct.includes('text/html')){
          ...
          const injected=text.replace(/<head[^>]*>/i, `$&<base href="${target}"><script>/* injected */</script>`);
        ...
        } else return new Response(resp.body,{status:resp.status,statusText:resp.statusText,headers});
      }catch(e){ return new Response('Proxy fetch failed: '+e.message,{status:502}); }
    })());
    return;
  }
